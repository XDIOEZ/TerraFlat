...
- 在中间显示简化 2D 预览，支持 `SpriteRenderer`、UI `Image` 和 `RawImage` 的图片资源。
...
2D 预览是近似绘制：UI 使用 RectTransform 的像素坐标，Sprite 使用 Transform 的世界坐标换算；Shader、粒子、脚本、动画和 Sprite 图集切片暂不执行。
