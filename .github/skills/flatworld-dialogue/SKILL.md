...
- `CharacterSpeechFacts` 集中维护已注册 Fact；当前为 `hunger.rate`、`hunger.tier`、`hunger.isTakingDamage`。
...
- `once=true` 必须有 `completionFlag`；完成标记写入玩家现有 `Data_Player.ItemSpecialData` 的 `flatworld.dialogue.completed`，不要扩展旧 MemoryPack 核心字段或新增独立存档。
- `Player.prefab` 根节点挂载 `ConfiguredSpeechProvider`，由 `RebuildExtensions()` 自动发现；不要给 Controller 添加手工 Provider 引用。
- 远程联机核心 Player 未执行 `Item.Load()`，Controller 会停在初始化门，因此远程视觉副本不会进入本地自言自语调度。
