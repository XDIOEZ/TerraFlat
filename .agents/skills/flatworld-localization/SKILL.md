...
5. `Assets/5_Scripts/5-3_GamePlay/Core/Manager/GameManager.UI.cs`：主菜单语言下拉、世界加载和难度动态文本。
6. `references/file-map.md`：按需读取的路径、资源和跨系统入口地图。
...
- UI 自动绑定只增加本地化组件，不改变 Prefab 层级、尺寸、锚点、字体、颜色或布局；布局问题另用 `flatworld-ugui-layout`。
- 业务逻辑保存稳定 ID、数值和状态，不保存翻译后的显示文本。显示层才调用本地化服务。
- 玩家自由输入、玩家聊天和用户生成文本不得作为静态 UI 翻译；角色自言自语的配置台词需要走单独的对话本地化方案。
...
7. 手动验证至少覆盖：主菜单语言切换、一个静态面板、一个动态设置面板、世界加载状态、物品名称/说明；切换回中文后确认 fallback 和事件没有重复订阅。
8. 若新增了权威入口、资源目录、表名、key 契约或跨系统依赖，立即更新本 Skill 的路径地图与“近期变更”。
...
- 将 UI、物品、对话都塞进一个巨大表，造成 key 冲突、翻译职责混乱和加载范围不可控。
- 为翻译修改 Prefab 布局、复制一套英文节点或改变控件命名契约；本地化只改变文本内容，溢出/字体/RTL 才另行处理。
- 把玩家聊天、命令、玩家名或 MOD 用户输入自动送进固定翻译表；这些文本不是静态本地化资源。
...
4. 保持 `CharacterSoliloquyController`、Presenter、触发条件和存档完成标记不变；语言切换不应重置台词冷却或 `once` 状态。
5. 对缺失翻译记录编辑器校验并回退中文；玩家手动聊天仍显示原始输入。
