...
2. 绘图前查看 `Assets/6_Art/Generated/Actors/`：若有同角色立绘、概念图或运行时 Sprite，实际打开以锁定身份；再打开该目录下已使用的其他角色立绘比较画风。目录中缺少可用立绘时再选项目其他角色立绘，不把像素 Sprite 的处理方式套到高清立绘。
3. 为每张参考图声明职责：`identity reference` 决定角色是谁，`art-style and presentation reference` 只决定表现语言，`pose reference` 只决定姿势。冲突时身份参考优先。
4. 先列出必须保持的身份不变量：年龄感、体型、头发轮廓、面部印象、服装结构、主配色、标志配件和职业气质。缺失细节只做最少量、可解释的高清化。
5. 默认生成单人、自然平视、竖版全身立绘；头顶、双脚和身份配件完整可见，四周保留约 `8%-12%` 安全留白。用户或 UI 契约另有要求时服从实际消费方。
6. 使用透明背景流程生成无场景、地面、投影、边框、文字、Logo 或水印的 PNG；立绘允许平滑 Alpha，禁止套用像素素材的硬 Alpha 和颜色数限制。
7. 保存到 `Assets/6_Art/Generated/Actors/<Name>/`，默认命名为 `<Name>_Portrait_FullBody_<Expression>.png`；除非用户明确要求替换，不覆盖旧素材。
8. 在原尺寸和目标 UI 缩放尺寸下检查面部、完整轮廓、遮挡关系、身份一致性与透明边缘；视觉问题不得仅凭文件尺寸或脚本统计判定通过。
9. 仅在用户要求接入 UI 时配置 Unity：使用适合缩放的平滑过滤、Alpha 和关闭 Mipmap；禁止使用运行时小人的 `PPU 16`、Point 过滤、底部中心世界 Pivot 或 Sprite Sheet 切片。
