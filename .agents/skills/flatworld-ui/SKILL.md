...
- 橙色焦点框由 `FlatWorldUIFeedback`、`GameSaveItemView` 和槽位 UI 显示；`FlatWorldUI/Navigate` 只绑定手柄，键鼠模式的 W/A/S/D 必须只控制玩家移动、不能改变 EventSystem 焦点。`FlatWorldUI/Cancel` 的键盘入口只能是 Esc，B 必须保留给背包开关。主菜单没有 Player 时由 `GamepadUIRuntimeController` 直接检测键鼠/手柄输入切换运行模式。
...
- 统一重建器：`Assets/Editor/FlatWorld/PrefabBuilders/UI/RuntimeUIPrefabBuilder.cs`；全量菜单为 `FlatWorld/UI/Rebuild Runtime Prefab UI`，仅重建流送设置使用 `FlatWorld/UI/Rebuild World Streaming Settings UI`，坐标 HUD 使用 `FlatWorld/UI/Rebuild Player World Coordinate HUD`，显示设置与列表分页使用 `FlatWorld/UI/Rebuild Coordinate Display Settings UI`；后两个入口会同步登记新增的 `Prefab` Addressables 条目，避免无关 Prefab 重写。
...
- 2026-08-09：修复 B 键背包关闭被右上角常驻菜单抢先消费：`FlatWorldUI/Cancel` 不再绑定键盘 B，`Info_Button_List` 保留手柄导航但退出全局取消栈；`UISmokeTests` 覆盖该按键隔离。
- 2026-08-09：修正键鼠模式的手柄化：橙色焦点框仍保留，但 `FlatWorldUI/Navigate` 已彻底移除 W/A/S/D；键盘移动不再切换背包或菜单焦点，手柄导航不受影响。
- 2026-08-09：新增 `UI_PlayerWorldCoordinate` 常驻 HUD；Player Prefab 上的 `PlayerWorldCoordinateHUD` 只为本地玩家在 `PanelRoot` 左上创建卡片，实时显示 Transform X/Y，节点均不拦截输入，重建器会同步登记 `Prefab` Addressables。
- 2026-08-09：正式 UI 接入 `FlatWorldUI` 独立 String Table；`FlatWorldLocalizationSetup` 扫描 UI Prefab 与运行时模板，`BasePanel`/主题动态文本通过 `LocalizedTextBinder` 自动跟随 Locale 刷新，不改变 Prefab 布局结构。
- 2026-08-09：F4 GM 玩家页新增动态“管理员无敌：开/关”按钮；未开启管理员权限时禁用，切换后立即刷新标签、颜色与状态提示，GM Canvas 仍为开发者专用运行时例外。
- 2026-08-09：F4 GM 物品空投目录的物品按钮网格改为水平居中，减少右侧空白，不影响 AI 召唤网格。
- 2026-08-09：角色说话气泡固定为 `PanelRoot` 最低子层级，取消显示时置顶行为；背包与所有交互面板始终覆盖气泡。
- 2026-08-09：`UI_GameSaveManager` 的动态存档/角色条目改为自动导航；打开时焦点落在首个存档，选世界后转到首个角色或名称输入框，选角色后转到“进入世界”，刷新/删除时会清理失效 EventSystem 焦点并保留已确认条目的视觉状态。
- 2026-08-09：UI 新增面向玩家的文字时必须联动 `flatworld-localization`；静态文字同步 `FlatWorldUI` 表，动态文字登记 `EnglishUiOverrides`，保留中文 fallback 并执行默认表同步菜单。


## 修改后自动测试

- 基础测试脚本：`Assets/GameTest/UI/UISmokeTests.cs` 与 `Assets/GameTest/UI/WorldTopologyUISmokeTests.cs`；当前覆盖 UIManager、BasePanel 手柄导航/取消契约、运行时 UI 导航不绑定键盘移动键且 Cancel 不抢键盘 B、存档动态条目的焦点/选择态和自动导航、按键绑定双分页节点、Resources UIRoot、八个设置 Prefab（含坐标显示和主菜单设置）、主菜单设置入口、设置列表三分页、流送性能入口、按键行 Prefab、世界加载 Prefab、玩家坐标 HUD 的节点/左上锚点/输入穿透/Player 绑定、新世界难度命名契约，以及可选世界种子输入框的命名与卡片边界；联机 Prefab 与 GameRes 加载约束由 `NetworkingSmokeTests.cs` 覆盖。
