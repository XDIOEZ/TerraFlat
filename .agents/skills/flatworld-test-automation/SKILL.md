...
- 默认不运行 Test Runner、多进程联机或完整测试。文档、整理、资源移动且 GUID/行为不变、小范围可静态证明的改动，只做差异/编码检查、相关程序集编译和 Unity Console。
...
- 分类从领域 Skill 选择；`Smoke` 保持每领域关键检查。
- Actor JSON 静态覆盖在 `AI.Smoke`，MOD Actor DTO/Lua 外壳覆盖在 `Modding.Smoke`。
