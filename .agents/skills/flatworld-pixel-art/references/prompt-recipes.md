...
Input images: Image 1 is the closest project animal or monster reference and defines species silhouette, scale, and viewpoint; Image 2 is the merchant anchor and only defines shared color clustering and dark-outline language.
...
- 一次只修正一个可观察问题，例如“让眼睛在 16×16 画布下保留”或“把轮廓减为 1 像素”。
- 每轮重复身份、调色板、比例、背景、画布和对齐约束，不因进入迭代而省略硬规则。
