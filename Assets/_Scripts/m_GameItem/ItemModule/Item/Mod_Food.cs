...
public partial class Mod_Food : Module
{
    ...
    {
        //TODO 不使用字段决定是否显示面板 而是通过检测是否存在GameObject来决定
        if (PanleInstance != null) return; // 面板已存在则返回
        ...
        RefreshUI();
        Data.ShowCanvas = true; // 确保状态同步
    }

    [Button("隐藏面板")]
    public void HidePanle()
    {
        if (PanleInstance == null) return; // 面板不存在则返回

        Destroy(PanleInstance);
        DataUpdate -= RefreshUI;
        UIValues = null;
        Data.ShowCanvas = false; // 确保状态同步
    }
