    ...
    public static partial class FlatWorldLocalizationSetup
    {
        ...
        private static readonly Dictionary<string, string> EnglishDescriptionOverrides =
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "Meat_Cured", "Longer shelf life, with freshness inherited from the raw meat. Fresh ingredients last 9 default game days." },
                { "HerbPoultice", "Restores 10% maximum health over 30 seconds. Healing still requires protein." },
                { "MinersNote", "Copper tools can mine iron. Make simple iron tools with raw iron, wooden handles and rope; alloy copper and tin at nine to one for bronze." },
                ...
                { "Coconut_Shell", "A coconut shell that works as a simple reusable water vessel." },
                ...
                { "WoodAsh", "Ash left after burning plant matter, used as a lightweight basic processing material." },
        ...
        private static readonly Dictionary<string, string> EnglishUiOverrides =
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "燃烧中", "Burning" },
                { "已熄灭", "Extinguished" },
                { "{0}　燃料 {1:0} / {2:0} 秒", "{0}  Fuel {1:0} / {2:0} s" },
                { "燃料 / 火种", "Fuel / Tinder" },
                { "拖入燃料补充燃料；熄灭后拖入火种可重新点燃。", "Drag in fuel to refill it. When extinguished, drag in tinder to relight it." },
                { "没有燃料，火种无法点燃。", "There is no fuel, so tinder cannot light it." },
                { "已重新点燃。", "Relit." },
                { "拖入燃料补充燃料；火把熄灭后拖入火种可重新点燃。", "Drag in fuel to refill it. When extinguished, drag in tinder to relight the torch." },
                { "火把里没有燃料，火种无法点燃。", "The torch has no fuel, so tinder cannot light it." },
                { "火把重新点燃了。", "The torch has been relit." },
                { "这个物品不能作为燃料。", "This item cannot be used as fuel." },
                { "燃料已经满了。", "Fuel is already full." },
                { "已补充燃料。", "Fuel replenished." },
                { "火把的燃料已经满了。", "The torch is already full of fuel." },
                { "已补充火把燃料。", "Torch fuel replenished." },
                { "倒空", "Empty Jar" },
                ...
                { "一次只装一种液体；拖入液体原料或其他容器可装液，拖动容器可倾倒。", "Holds one liquid at a time. Drag in a liquid ingredient or another vessel to fill it; drag the vessel to pour." },
                ...
                { "矿层太硬，需要更高级的镐。", "This deposit requires a higher-tier pickaxe." },
                { "装好了海水，可以加热制盐。", "Filled with seawater. Heat it to make salt." },
                { "装好了淡水，烧开后再喝。", "Filled with fresh water. Boil it before drinking." },
                { "又热又渴，先补水，到凉快的地方休息。", "Too hot and thirsty. Drink water and rest somewhere cool." },
                { "流汗太多，体力上限下降了，吃些含盐食物吧。", "Sweating has lowered my stamina capacity. I need some salty food." },
                { "原料已经腐败，无法腌制。", "The ingredient has spoiled and cannot be cured." },
                { "这里没有可拆的平台。", "There is no removable platform here." },
                { "请先移走平台上的建筑。", "Remove the buildings on the platform first." },
                { "请先移走平台上的角色和物品。", "Clear people and items from the platform first." },
                ...
                { "太阳长投影", "Sun Shadows" },
                { "阴影柔化", "Shadow Softening" },
                { "模糊程度", "Blur Strength" },
                { "地面层级阴影", "Ground Elevation Shadows" },
                { "阴影宽度", "Shadow Width" },
                { "随太阳方向投射长阴影。低配设备可关闭以减少渲染开销。", "Cast long shadows following the sun. Disable on lower-end devices to reduce rendering cost." },
                ...
                { "写实：柔和水色、连续波浪与细碎反光。", "Realistic: softer colors, flowing waves and subtle glints." },
                { "保存", "Save" },
                { "返回游戏主界面", "Return to Main Menu" },
                { "返回桌面", "Exit to Desktop" },
                { "是否保存再退出", "Save before exiting?" },
                { "退出游戏", "Exit Game" },
                ...
                { "开启：保持存档原有生成规则。关闭：跟随当前版本并升级生成基线；已探索与未探索区域的基础地形和自然生成都可能变化，玩家差量仍保留。重新开启后会在下次进入世界重新冻结。", "On: keep this save's existing generation rules. Off: follow the current game version and upgrade the generation baseline; base terrain and natural generation may change in both explored and unexplored areas, while player deltas are retained. Re-enable it to freeze the current rules the next time you enter the world." },
                ...
                { "双指缩放灵敏度", "Pinch Zoom Sensitivity" },
                ...
                { "作业状态", "Work Status" },
                { "当前炉温 {0}°C", "Current furnace temperature {0}°C" },
                { "投入", "Input" },
                { "产出", "Output" },
                ...
                { "放入材料 · 选择配方 · 开始制作", "Add materials · select a recipe · start crafting" },
                { "输入材料", "Input Materials" },
                { "工具", "Tool" },
                { "可制作配方", "Craftable Recipes" },
                { "制作产物", "Crafted Outputs" },
                { "制作台", "Workbench" },
                ...
                { "手机触屏控制", "Mobile Touch" },
                { "触屏布局", "Touch Layout" },
                { "拖动固定摇杆和玩法按钮；位置会按安全区域自适应不同屏幕。", "Drag fixed joysticks and gameplay buttons; positions adapt to each screen's safe area." },
                { "拖动控件后点击保存。", "Drag controls, then select Save." },
                { "拖动按钮和摇杆到想要的位置；保存后会立即应用。", "Drag buttons and joysticks where you want them; saving applies the layout immediately." },
                { "已恢复默认预览；点击保存后生效。", "Default layout restored in the preview; select Save to apply it." },
                { "触屏布局资源尚未准备好。", "Touch layout resources are not ready yet." },
                { "触屏布局编辑器组件缺失。", "The touch layout editor component is missing." },
                { "正在编辑触屏按钮布局。", "Editing the touch control layout." },
                { "无法打开触屏布局编辑器。", "Unable to open the touch layout editor." },
                ...
                { "季节长度", "Season Length" },
                { "游戏天数", "Game days" },
                { "一年共 24 天", "24 days per year" },
                { "默认每季 6 天，一年 24 天。可分别调整四季，当前季节进度会保留。", "By default, each season lasts 6 days and a year lasts 24 days. Adjust seasons separately while keeping current seasonal progress." },
                { "一年共 {0} 天；调整会保留当前季节进度。", "{0} days per year. Changes preserve current seasonal progress." },
                { "每个季节的天数必须是大于 0 的有限数值。", "Each season must have a finite length greater than zero." },
                { "请在自己管理的世界中调整季节。", "Season lengths can only be changed in a world you manage." },
                { "请先进入世界。", "Enter a world first." },
                ...
                { "双指缩放灵敏度为 0 时关闭，数值越大响应越快。镜头前探正值为提前跟随，负值为惯性；缩放影响系数为正时拉远会增强预测，为负时会减弱。", "Pinch zoom is disabled at 0; higher values respond faster. Positive lookahead moves the camera ahead; negative values add inertia. Positive zoom influence strengthens prediction when zoomed out, while negative values weaken it." },
                ...
                { "重置绑定失败。", "Failed to reset the binding." },
                { "“{0}”已恢复默认绑定。", "“{0}” was restored to its default binding." },
                { "其他操作", "another action" },
