    ...
    public static partial class FlatWorldLocalizationSetup
    {
        ...
        private static readonly Dictionary<string, string> EnglishUiOverrides =
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                { "饮水", "Drink" },
                { "倒空", "Empty Jar" },
                { "从手持罐倒入", "Pour from Held Jar" },
                { "手持陶罐对准水域使用即可装水；放入炉子输入槽烧水或制盐。", "Use the held jar on water to fill it. Place it in a furnace input slot to boil water or make salt." },
                { "{0}　{1} / {2} 份\n加热进度：{3:0} 秒", "{0}  {1} / {2} servings\nHeating: {3:0} s" },
                { "不同水质不能混装，请先倒空陶罐。", "Empty the jar before filling it with a different type of water." },
