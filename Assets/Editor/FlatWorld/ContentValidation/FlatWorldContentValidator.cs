...
public static class FlatWorldContentValidator
{
    ...
    private const string RecipeExcelAssetPath = "Assets/GameConfig/Excel/RecipeConfig.xlsx";
    ...
    private static void ValidateRecipes(ValidationContext context, FlatWorldContentValidationReport report)
    {
        ...
        try
        {
            RecipeExcelSyncService.ValidateExcelMenu();
        }
        catch (Exception exception)
        {
            AddError(report, "FWC-RECIPE-009", RecipeExcelAssetPath, "Workbook", $"配方 Excel 校验失败：{exception.Message}", null);
        }
    }
