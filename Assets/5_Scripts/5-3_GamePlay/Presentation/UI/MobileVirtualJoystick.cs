    ...
    IPointerUpHandler
{
    ...
    private void Update()
    {
        if (HasPointerOwnership && MobileHeldItemDropSurface.HasPlayerHeldItem())
            ResetOwnership();
    }
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        // 手持物触点由同节点的 MobileHeldItemDropSurface 独立持有，摇杆不再复制丢弃手势状态。
        if (MobileHeldItemDropSurface.HasPlayerHeldItem())
            return;
