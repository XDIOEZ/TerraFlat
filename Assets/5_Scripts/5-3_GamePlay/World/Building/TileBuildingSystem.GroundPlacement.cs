...
public static partial class TileBuildingSystem
{
        ...
        out RuntimeTerrainTileSample sample, out int tileId, out string reason)
    {
        ...
        float liquidDepth = sample.Terrain.GetLiquidDepth(sample.LocalCell.x, sample.LocalCell.y);
        GroundLiquidRequirement liquidRequirement = definition.groundPlacement.LiquidRequirement;
        if (liquidRequirement == GroundLiquidRequirement.Liquid && liquidDepth <= 0f)
        {
            reason = "水上平台只能铺在液体上";
            return false;
        }
        if (liquidRequirement == GroundLiquidRequirement.Dry && liquidDepth > 0f)
        {
            reason = "地板只能铺在无液体地面";
            return false;
        }
